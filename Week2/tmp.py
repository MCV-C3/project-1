...
def train_simple_model(model, model_name, train_loader, test_loader, search_type=None, param_value=None):
...
for image_size in [224, 14, 28, 56, 112]:
    ...
    image_search_results[input_size] = train_simple_model(
        ...
        search_type=f"image_size_search_{image_size}", param_value=image_size
    )
...
for hidden_layers_n in [1, 2, 3, 4, 5, 6, 7, 8]:
    ...
    layer_search_results[hidden_layers_n] = train_simple_model(
        model, model_name, train_loader, test_loader,
        search_type=f"layer_search_{hidden_layers_n}", param_value=hidden_layers_n
    )
...
for hidden_dim in [50, 100, 150, 200, 250, 300, 350, 400, 450, 500]:
    ...
    dimension_search_results[hidden_dim] = train_simple_model(
        model, model_name, train_loader, test_loader,
        search_type=f"dimension_search_{hidden_dim}", param_value=hidden_dim
    )
...
for patch_size in [14, 28, 56, 112]:
    print(f"\n{'='*50}")
