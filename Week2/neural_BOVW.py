...
train_feats, train_labels = extract_patch_features(model, train_loader, device, 1,patch_size)
test_feats, test_labels = extract_patch_features(model, test_loader, device, 1,patch_size)
print(train_feats.shape)
print(train_labels.shape)
acc , _ = neural_based_fisher(train_feats, test_feats, train_labels, test_labels)

print(f"BOVW accuracy using layer {1}: {acc:.4f}")