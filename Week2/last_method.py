...
def neural_based_fisher(train_descriptors, test_descriptors, train_labels, test_labels):
    ...
    print("mamaMia")
    # Train classifier (Logistic Regression) and evaluate with CV and test
    clf = LogisticRegression(solver='lbfgs', class_weight='balanced', max_iter=15_000, n_jobs=-1)
    print("pizzeria")
    clf.fit(X_train_scaled, y_train)
    print("Fitted")
    y_pred = clf.predict(X_test_scaled)
...
if __name__ == "__main__":
    ...
    for layer_id in range(hidden_layers_n+1):
        ...
        print(neural_based_fisher(train_feats, test_feats, train_labels, test_labels))
